...
public class Config
{
    ...
    public int PositionYSeparation { get; set; } = 75;
    ...
    private static int ParseIntEnv(string key, bool required, int defaultValue = 0)
    {
        ...
        {
            if (required) throw new InvalidOperationException($"{key} is required");
            return defaultValue;
        }

        if (!int.TryParse(val, out var intVal)) throw new InvalidOperationException($"{key} must be a valid integer");

        if (intVal <= 0) throw new InvalidOperationException($"{key} must be positive, got {intVal}");
    ...
    private void Validate()
    {
        ...
        if (PollIntervalSeconds < 15)
            throw new InvalidOperationException(
                $"POLL_INTERVAL_SECONDS must be at least 15 seconds to prevent API spamming, got {PollIntervalSeconds}");
    }
}